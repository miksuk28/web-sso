...
class UsersDatabaseWrapper:
    ...
    def _list_to_dicts(keys, data, return_one=False):
        total_list = []
        for i in range(len(data)):
            local_dict = {}
            for j in range(len(keys)):
                local_dict[keys[j]] = data[i - 1][j]

            total_list.append(local_dict)
    ...
    def auth(self, username, password, service, expiration):
        ...
        token = jwt.encode({
            ...
            "expiration": int(self._unixtime() + self._token_validity)
        },
